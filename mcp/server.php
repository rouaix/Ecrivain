 ...
 * Config    : fichier .env dans le même répertoire (copier .env.example)
 *
...
function apiPost(string $path, array $body = []): mixed  { return apiRequest('POST',   $path, $body); }
function apiPut(string $path, array $body = []): mixed   { return apiRequest('PUT',    $path, $body); }
function apiDelete(string $path): mixed                  { return apiRequest('DELETE', $path); }
...
{
    global $API_URL, $API_TOKEN;
    if (!file_exists($filePath)) throw new RuntimeException("Fichier introuvable : $filePath");
    ...
    if ($status >= 400) throw new RuntimeException($data['error'] ?? "Erreur upload ($status).");
    return $data;
}
...
{
    $acts = $data['acts'] ?? [];
    if (!$acts) return '_Aucun acte dans ce projet._';
    $out = "# Actes (" . count($acts) . ")\n\n";
    foreach ($acts as $a) {
        $out .= "## {$a['title']} *(id: {$a['id']})*\n";
        if (!empty($a['description'])) $out .= $a['description'] . "\n";
...
{
    $out  = "# {$a['title']} *(id: {$a['id']})*\n";
    if (!empty($a['description'])) $out .= "\n{$a['description']}\n";
    if (!empty($a['resume']))      $out .= "\n**Résumé** : {$a['resume']}\n";
    $out .= "\n## Chapitres\n\n";
...
function mdChapter(array $ch): string
{
    ...
    if (!empty($ch['act_id'])) $out .= "**Acte id** : {$ch['act_id']}\n";
    $out .= "**Mots** : {$ch['word_count']} | **Mis à jour** : " . fmtDate($ch['updated_at']) . "\n";
    if (!empty($ch['resume']))       $out .= "\n## Résumé\n{$ch['resume']}\n";
...
function mdSections(array $data): string
{
    ...
    if (!$sections) return '_Aucune section dans ce projet._';
    $out = "# Sections (" . count($sections) . ")\n\n";
    foreach ($sections as $s) {
        $label = $s['type_label'] ?? $s['type'];
        $title = $s['title'] ?? '(sans titre)';
        $out  .= "## {$label} — {$title} *(id: {$s['id']})*\n";
        if (!empty($s['has_image'])) $out .= "_Contient une image_\n";
        $out .= "\n";
    }
    return $out;
}
...
function mdSection(array $s): string
{
    ...
    $title = $s['title'] ?? '(sans titre)';
    $out   = "# {$label} — {$title} *(id: {$s['id']})*\n";
    if (!empty($s['comment']))   $out .= "\n**Note** : {$s['comment']}\n";
    if (!empty($s['image_url'])) $out .= "\n**Image** : {$s['image_url']}\n";
...
{
    $notes = $data['notes'] ?? [];
    if (!$notes) return '_Aucune note dans ce projet._';
...
function mdCharacters(array $data): string
{
    ...
    if (!$characters) return '_Aucun personnage dans ce projet._';
    $out = "# Personnages (" . count($characters) . ")\n\n";
    foreach ($characters as $c) {
        $out .= "## {$c['name']} *(id: {$c['id']})*\n";
        if (!empty($c['description'])) $out .= substr($c['description'], 0, 100) . (strlen($c['description']) > 100 ? '…' : '') . "\n";
        $out .= "\n";
    }
    return $out;
}
...
{
    $out = "# {$c['name']} *(id: {$c['id']})*\n";
    if (!empty($c['description'])) $out .= "\n## Description\n{$c['description']}\n";
    if (!empty($c['comment']))     $out .= "\n## Notes\n{$c['comment']}\n";
    return $out;
}
...
function mdElements(array $data): string
{
    ...
    if (!$elements) return '_Aucun élément dans ce projet._';
...
function mdElement(array $e): string
{
    ...
    if (!empty($e['resume'])) $out .= "\n**Résumé** : {$e['resume']}\n";
    $out .= "\n## Contenu\n" . ($e['content_text'] ?: '_Contenu vide._') . "\n";
    if (!empty($e['sub_elements'])) {
        $out .= "\n## Sous-éléments\n\n";
        foreach ($e['sub_elements'] as $s) $out .= "- **{$s['title']}** *(id: {$s['id']})*\n";
    }
    return $out;
}
...
{
    $images = $data['images'] ?? [];
    if (!$images) return '_Aucune image dans ce projet._';
...
function mdSearch(array $data): string
{
    ...
    if (!$results) return "_Aucun résultat pour « {$q} »._";
    $out = "# Résultats pour « {$q} » (" . count($results) . ")\n\n";
    foreach ($results as $r) {
        $out .= "## {$r['title']} *({$r['type']}, id: {$r['id']})*\n";
        if (!empty($r['excerpt'])) $out .= "> …{$r['excerpt']}…\n";
        $out .= "\n";
    }
    return $out;
}
...
function buildTools(): array
{
    ...
    return [
        ...
        'delete_project' => [
            ...
            'inputSchema' => $req(['id' => $prop('integer', 'ID du projet')], ['id']),
            'handler'     => function($a) { apiDelete("/api/project/{$a['id']}"); return mdOk("Projet {$a['id']} supprimé."); },
        ],
        ...
        'delete_act' => [
            ...
            'inputSchema' => $req(['id' => $prop('integer', "ID de l'acte")], ['id']),
            'handler'     => function($a) { apiDelete("/api/act/{$a['id']}"); return mdOk("Acte {$a['id']} supprimé."); },
        ],
        ...
        'delete_chapter' => [
            ...
            'inputSchema' => $req(['id' => $prop('integer', 'ID du chapitre')], ['id']),
            'handler'     => function($a) { apiDelete("/api/chapter/{$a['id']}"); return mdOk("Chapitre {$a['id']} supprimé."); },
        ],
        ...
        'create_section' => [
            ...
            'handler' => fn($a) => mdSection(apiPost("/api/project/{$a['project_id']}/sections", array_filter([
                'type' => $a['type'], 'title' => $a['title'] ?? null, 'content' => $a['content'] ?? null, 'comment' => $a['comment'] ?? null,
            ]))),
        ],
        'update_section' => [
        ...
        'delete_section' => [
            ...
            'inputSchema' => $req(['id' => $prop('integer', 'ID de la section')], ['id']),
            'handler'     => function($a) { apiDelete("/api/section/{$a['id']}"); return mdOk("Section {$a['id']} supprimée."); },
        ],
        ...
        'delete_note' => [
            ...
            'inputSchema' => $req(['id' => $prop('integer', 'ID de la note')], ['id']),
            'handler'     => function($a) { apiDelete("/api/note/{$a['id']}"); return mdOk("Note {$a['id']} supprimée."); },
        ],
        ...
        'delete_character' => [
            ...
            'inputSchema' => $req(['id' => $prop('integer', 'ID du personnage')], ['id']),
            'handler'     => function($a) { apiDelete("/api/character/{$a['id']}"); return mdOk("Personnage {$a['id']} supprimé."); },
        ],
        ...
        'create_element' => [
            ...
            'handler' => fn($a) => mdElement(apiPost("/api/project/{$a['project_id']}/elements", array_filter([
                'title' => $a['title'], 'template_element_id' => $a['template_element_id'], 'parent_id' => $a['parent_id'] ?? null,
            ]))),
        ],
        'update_element' => [
        ...
        'delete_element' => [
            ...
            'inputSchema' => $req(['id' => $prop('integer', "ID de l'élément")], ['id']),
            'handler'     => function($a) { apiDelete("/api/element/{$a['id']}"); return mdOk("Élément {$a['id']} supprimé."); },
        ],
        ...
        'delete_image' => [
            ...
            'handler'     => function($a) { apiDelete("/api/project/{$a['project_id']}/image/{$a['image_id']}"); return mdOk("Image {$a['image_id']} supprimée."); },
        ],
        ...
        'search' => [
            ...
            'handler' => function($a) {
                $params = ['q' => $a['q']];
                if (!empty($a['project_id'])) $params['pid'] = $a['project_id'];
