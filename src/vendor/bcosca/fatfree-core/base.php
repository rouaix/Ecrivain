...
final class Base extends Prefab implements ArrayAccess
{
	...
	function set($key, $val, $ttl = 0)
	{
			...
			if ($expr[1] == 'COOKIE') {
				...
				if (isset($_COOKIE[$parts[1]]))
					call_user_func_array(
						'setcookie',
						array_values(array_merge([$parts[1], NULL], ['expire' => 0] + $jar))
					);
				if ($ttl)
					$jar['expire'] = $time + $ttl;
				call_user_func_array('setcookie', array_values([$parts[1], $val] + $jar));
				$_COOKIE[$parts[1]] = $val;
				return $val;
			}
		} else
		...
		if (preg_match('/^JAR\b/', $key)) {
			if ($key == 'JAR.lifetime')
				...
				unset($jar['expire']);
				if (!headers_sent() && session_status() != PHP_SESSION_ACTIVE)
					call_user_func_array('session_set_cookie_params', array_values($jar));
			}
		}
		if ($ttl)
	...
	function __construct()
	{
		...
		$check = error_reporting((E_ALL | E_STRICT) & ~(E_NOTICE | E_USER_NOTICE | E_DEPRECATED | E_USER_DEPRECATED));
		...
		if (ini_get('auto_globals_jit'))
			// Override setting
			if (!array_key_exists('_ENV', $GLOBALS))
				$GLOBALS['_ENV'] = $_ENV;
		if (!array_key_exists('_REQUEST', $GLOBALS))
