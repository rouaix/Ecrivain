...
class XLSX {
     ...
     * @param array $headers
     */
    function addSheet($name,$rows,$headers=[]) {
        if (!$name)
            $name=self::DEFAULT_sheet;
        if ($headers)
            $this->writer->writeSheetHeader($name,$headers,gettype(key($headers))=='integer');
        $this->writer->writeSheet($rows,$name);
    }
     ...
     * @param bool $die
     */
    function output($filename='output.xlsx',$die=TRUE) {
    ...
    function __construct() {
        $this->writer=new XLSXWriter(Base::instance()->get('TEMP'));
    }
...
class XLSXWriter
{
    ...
    protected function writeCell(XLSXWriter_BuffererWriter &$file, $row_number, $column_number, $value, $cell_format_index)
    {
        ...
        if (!is_scalar($value) || $value==='') { //objects, array, empty
            ...
            $file->write('<c r="'.$cell_name.'" s="'.$cell_format_index.'" t="n"><v>'.str_replace(',','.',self::convert_date_time($value)).'</v></c>');// [xfra] Fix float cast to string on non-English locales
