...
/**
 * ExportContentService — generates export content for text-based formats
 * (txt, html, markdown, vector, clean, summaries).
 * Extracted from ProjectExportController::generateExportContent().
 */
class ExportContentService
{
    ...
    /**
     * Returns ['content' => string, 'ext' => string, 'title' => string] or null on access error.
     */
    public function generate(int $pid, string $format): ?array
    {
        $db = $this->f3->get('DB');
        ...
        // Closure to append one content item
