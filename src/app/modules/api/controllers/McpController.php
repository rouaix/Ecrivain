...
class McpController extends Controller
{
    ...
    private function callTool(string $name, array $a): array
    {
        ...
        try {
            return match ($name) {
                ...
                'get_synopsis'     => $this->toolGetSynopsis($uid, (int) ($a['project_id'] ?? 0)),
                'update_synopsis'  => $this->toolUpdateSynopsis($uid, (int) ($a['project_id'] ?? 0), $a),
    ...
    private function toolGetSynopsis(int $uid, int $pid): array
    {
        if (!$this->ownsProject($uid, $pid)) return $this->fail("Projet $pid introuvable.");
    ...
    private function toolUpdateSynopsis(int $uid, int $pid, array $a): array
    {
        if (!$this->ownsProject($uid, $pid)) return $this->fail("Projet $pid introuvable.");
