...
class AiService extends Prefab
{
    ...
    public function generate(string $systemPrompt, string $userPrompt, float $temperature = 0.7)
    {
        ...
        switch ($this->provider) {
            case 'gemini':
                return $this->generateGemini($systemPrompt, $userPrompt, $temperature);
            case 'anthropic':
                return $this->generateAnthropic($systemPrompt, $userPrompt, $temperature);
            case 'mistral':
                // Mistral is compatible with OpenAI API
                return $this->generateOpenAI($systemPrompt, $userPrompt, $temperature, 'https://api.mistral.ai/v1/chat/completions');
            case 'openai':
            default:
                return $this->generateOpenAI($systemPrompt, $userPrompt, $temperature);
        }
    }

    private function generateOpenAI($systemPrompt, $userPrompt, $temperature, $url = 'https://api.openai.com/v1/chat/completions')
    {
        $data = [
            'model' => $this->model,
            'messages' => [
    ...
    private function generateGemini($systemPrompt, $userPrompt, $temperature)
    {
    ...
    private function generateAnthropic($systemPrompt, $userPrompt, $temperature)
    {
