...
class AiController extends Controller
{
    ...
    /**
     * Génère un synopsis complet depuis une idée en quelques lignes.
    ...
    public function synopsisFromIdea()
    {
        ...
        $userConfig = $this->getUserConfig();
        $provider  = $userConfig['active_provider'] ?? 'openai';
        $apiKey    = $userConfig['providers'][$provider]['api_key'] ?? '';
        $model     = $userConfig['providers'][$provider]['model'] ?? 'gpt-4o';
        $service   = new AiService($provider, $apiKey, $model);
    ...
    /**
     * Génère un synopsis complet depuis le contenu du projet existant.
    ...
    public function synopsisFromProject()
    {
        ...
        // Personnages (top 5)
        $chars = $db->exec(
            'SELECT name, description FROM characters WHERE project_id=? ORDER BY id ASC LIMIT 5',
            [$pid]
        );
        $charsText = '';
        foreach ($chars as $c) {
            ...
            if (is_array($decoded)) {
                // Description stockée en JSON structuré — extraire les champs textuels utiles
                $parts = [];
        ...
        // Chapitres avec résumés (priorité) ou extrait
        $chapters = $db->exec(
            'SELECT title, resume, content FROM chapters WHERE project_id=? AND parent_id IS NULL ORDER BY order_index ASC, id ASC',
            [$pid]
        );
        $chaptersText = '';
        $budget = 5000;
        foreach ($chapters as $ch) {
            if ($budget <= 0) break;
            if (!empty($ch['resume'])) {
                $excerpt = mb_substr(strip_tags($ch['resume']), 0, 600);
            } else {
                $excerpt = mb_substr(strip_tags(html_entity_decode($ch['content'] ?? '')), 0, 300);
            }
            if (!$excerpt) continue;
        ...
        $provider   = $userConfig['active_provider'] ?? 'openai';
        $apiKey     = $userConfig['providers'][$provider]['api_key'] ?? '';
        $modelName  = $userConfig['providers'][$provider]['model'] ?? 'gpt-4o';
        $service    = new AiService($provider, $apiKey, $modelName);

        $systemPrompt = "Tu es un expert en dramaturgie. Tu analyses le contenu d'un roman et tu rédiges un synopsis structuré complet. Tu réponds UNIQUEMENT avec un objet JSON valide contenant exactement ces 16 clés :\n{\n  \"genre\": \"ex: Thriller\",\n  \"subgenre\": \"ex: Techno-thriller\",\n  \"tone\": \"ex: Sombre, haletant\",\n  \"themes\": \"ex: Vérité, manipulation, IA\",\n  \"comps\": \"ex: Dans l'esprit de 1984 de Orwell\",\n  \"audience\": \"adulte|ya|mg|enfant\",\n  \"logline\": \"une phrase\",\n  \"pitch\": \"1-2 paragraphes\",\n  \"situation\": \"texte\",\n  \"trigger_evt\": \"texte\",\n  \"plot_point1\": \"texte\",\n  \"development\": \"texte\",\n  \"midpoint\": \"texte\",\n  \"crisis\": \"texte\",\n  \"climax\": \"texte\",\n  \"resolution\": \"texte\"\n}\nAucun autre texte, aucun markdown, aucune autre clé JSON.";
        ...
        $this->logAiUsage($modelName, $result['prompt_tokens'] ?? 0, $result['completion_tokens'] ?? 0, 'synopsis_from_project');
    ...
    public function synopsisGenerateBeat()
    {
        ...
        foreach ($context as $key => $val) {
            if (!empty($val) && $key !== $beat && in_array($key, $validBeats, true)) {
                $label = $beatLabels[$key] ?? $key;
                $contextText .= "$label : " . mb_substr(strip_tags($val), 0, 300) . "\n";
            }
        }
    ...
    public function synopsisSuggestLogline()
    {
        ...
        $systemPrompt = "Tu es spécialiste de la logline cinématographique et romanesque. Une logline = 1 phrase : personnage + objectif + obstacle + enjeu. Tu génères des loglines percutantes, sans clichés. Réponds UNIQUEMENT par un tableau JSON de 3 chaînes, sans autre texte.";
